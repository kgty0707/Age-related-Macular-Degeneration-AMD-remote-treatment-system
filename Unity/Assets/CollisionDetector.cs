...
public class CollisionDetector : MonoBehaviour
{
    ...
    public string CollidingObjectTag { get; private set; } // 현재 충돌 중인 객체의 태그 저장
    public GameObject CollidingObject { get; private set; } // 현재 충돌 중인 객체 저장
    public Vector3 EntryDirection { get; private set; }  // 충돌의 진입 방향 저장
    private Color originalColor; // 객체의 원래 색상 저장
    private Renderer collidingObjectRenderer; // 충돌 객체의 렌더러 컴포넌트
    public int count = 0; // "sphere"가 아닌 객체와의 충돌 횟수 카운터
    ...
    public GameObject bloodSprayFX; // 피 분수
    public GameObject bloodSprayFX2; // 피 분수 다른 모양 
        ...
        if (collidingObjectRenderer != null)
        {
            ...
            else if (other.CompareTag("eye")) // 목표점 이외의 eye와 충돌했을 경우 삐빅 소리내고 틀린 횟수 기록 
            {
                ...
                if (count >= 20)
                {
                    ...
                    bloodSprayFX.SetActive(true); // blood 효과 
                    bloodSprayFX2.SetActive(true); // blood 효과 
                }
                eyeCollided = true;
            }
                else
